...
4. 当前仓库主要维护以下能力:
   ...
   - `dependency-boundary-design`: 在同一责任的工具、库、SDK、存储或基础设施调用分散、依赖处理重复或漂移, 或同类变化需要跨多个位置处理时, 判断是否建立明确边界; 需要时按责任完成调用收口, 让后续变化集中处理, 多实现能力只在现实义务出现后演进。
   - `minimal-implementation`: 在目标、contract 和责任 owner 已明确后, 识别会改变实现选择的维护面, 并在正确候选之间选择或报告总体维护面更小的方案。
   ...
   - `ai-ready-docs`: 独立组成一个分发单元, 将说明、prompt、agent 指令、规则、任务、需求、模板和工作流等文档优化到适合 AI 阅读、理解和使用的状态, 并保持便于人维护的阅读脉络。
