...
- 决策: case 使用固定标题和文字 `Contract:`；main、derived、exempt 与发现器识别的测试入口一一绑定但允许多个入口归入同一 case；Scope 使用严格 glob 和 Git 可见路径校验，并依据工作区变化、最近 CR 结果与提交基线返回 review trigger。
