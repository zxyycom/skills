...
- 让 active stage 只表达 Change 的内容成熟度，任务进度由 tasks 表达，完成结果由 archive 目录表达。
...
- 采用: CLI 只提供 `list`、`show`、`check`、`check-all`、`plan` 与 `archive`。`plan` 用于 `draft -> plan`，也允许在语义复核现有 Plan 后重新确认并刷新基线；`archive` 只接受结构有效、全部任务完成且已经通过语义审阅与归档授权的 active Plan。
