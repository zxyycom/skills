...
## 索引摘要
- 目的: 让工具、索引或写入故障恢复到当前自包含 Markdown 与 schema v4 的单一可校验状态。
- 背景: Markdown 已拥有全部权威状态，继续维护旧 schema 映射或独立索引状态会重新引入并行事实源和兼容路径。
- 决策: 恢复手册只处理当前格式；优先恢复当前 CLI，再从 Markdown 重建 schema v4 索引，不读取、迁移或推断其他 schema。
...
- schema v4 索引只是从全部有效 Markdown 生成的查询投影，缺失或损坏时不需要从旧索引恢复成员或状态。
...
- 采用: 索引缺失、损坏或投影漂移时，只从全部有效当前格式 Markdown 生成完整 schema v4 索引，不从索引反向覆盖 Markdown。
