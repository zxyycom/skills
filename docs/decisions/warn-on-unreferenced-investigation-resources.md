...
- 完全未引用的文件可能是待接入材料或待清理孤儿。Warning 足以暴露问题，失败退出会把资源池卫生扩大成所有调查维护的全局前置。
...
- 采用: 按既有版本控制可见性规则，被 ignore 排除的未跟踪文件不产生 orphan warning，报告引用它时失败；tracked 或显式进入 pending 的 ignored 文件继续作为可见资源校验。
- 采用: Scoped check 只验证命中报告及其直接引用，不证明跨主题 owner 或全局 orphan 状态；默认全量 check 与 `sync-index` 才建立完整引用集合。
