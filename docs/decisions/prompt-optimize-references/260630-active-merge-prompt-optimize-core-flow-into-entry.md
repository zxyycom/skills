...
- 本决定替代此前将 `workflows.md` 和 `rewrite-rules.md` 作为主动 owner 的安排; [旧决策记录](260630-superseded-reorganize-prompt-optimize-rewrite-rules-as-pipeline.md) 继续保留, 用于回放上一阶段拆分的理由。
...
- 触发条件: 后续维护 `prompt-optimize` 时, 如果某个引用文件在主场景中几乎必读, 且缺失会改变执行质量或判断路径, 应优先考虑把主干并回入口, 再把原文件降级为低频参考或迁移期保留材料。
