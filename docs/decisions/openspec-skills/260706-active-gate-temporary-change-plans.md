...
- 采用: `skills/openspec-propose/SKILL.md` 必须保持阻塞级审计任务, 并放在所有实现任务之前。审计至少检查 proposal、design、specs 和 tasks 是否围绕核心句, capability ID 是否符合命名规则, change 是否只包含未审核临时 artifacts, 是否没有影响其它文档, 以及 `## Open Questions` 是否没有未回答问题或已收敛歧义。
