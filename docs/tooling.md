...
1. `bun run validate`: 校验全部 submodule skill 入口、内部链接、决策记录结构和主仓库项目配置。
2. `bun run validate:decisions`: 单独校验 `docs/decisions/` 的目录、文件结构、索引链接和状态来源链接目标。
3. `bun run pack:skills`: 将各子仓库 `skill/*/` 下每个 skill 分别打包为 `dist/<skill-name>.zip`。
4. `bun run check`: 先校验，再打包全部 skill。
...
2. 脚本优先覆盖所有 skill 的共同规则；确实存在 skill 专属规则时，在脚本中集中声明。
3. 脚本应优先使用 Bun 和 Node 兼容的标准 API，避免为简单校验和打包引入依赖。
4. 脚本默认只读写主仓库内路径；需要生成产物时输出到主仓库 `dist/`。
5. 打包脚本只打包子仓库 `skill/<skill-name>/`，不把项目文档、CI、脚本或仓库元数据放进 skill zip。
6. Markdown 链接提取使用 `mdast-util-from-markdown` 解析 Markdown AST；脚本负责仓库路径、状态来源和项目约束校验。
7. Markdown 内部链接目标必须是仓库内路径且目标存在；`#anchor` 必须匹配目标 Markdown 文件中的标题锚点。
8. 决策记录校验保留为独立脚本，总校验复用同一规则。
