...
1. `bun run validate`: 校验 skill 入口、内部链接、包目录和项目级配置。
2. `bun run pack:skill`: 将 `skill/git-commit-organizer/` 打包为 `dist/git-commit-organizer.zip`。
...
4. 打包脚本只打包 `skill/git-commit-organizer/`，不把项目文档、CI、脚本或仓库元数据放进 skill zip。
...
4. 运行 `bun run check`。
5. 上传 `dist/git-commit-organizer.zip` 作为 workflow artifact。
6. 对 `main` 分支的 `push` 和 `workflow_dispatch`，发布或更新 GitHub Release `git-commit-organizer-latest`，并上传 `dist/git-commit-organizer.zip`。
