...
8. 外部 JSON 在边界完成运行时收窄；同一结构由多个入口消费或需要稳定字段诊断时，由 `valibot` schema 同时承接结构和类型。单一内部生成值沿用其构造类型。
9. `decision-records` 的命令、参数、帮助和非法参数诊断由 `commander` 承接；短小的单命令脚本使用 Node 标准参数解析能力。
