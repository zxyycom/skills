...
Skill 是行为单元；分发单元是需要共同说明安装、更新、兼容性和交付关系的边界，不等于一个物理包或必须原子安装的集合。一个分发单元可以只有一个 skill，也可以聚合一组共同交付的 skill；聚合单元的成员仍可分别拥有制品、版本和 updater，并由使用者按需选装。同一单元内允许显式依赖和交接；跨单元依赖要写明前置条件和缺失路径。

在本项目中，`ai-ready-docs` 独立组成一个分发单元；四个 OpenSpec skills 共同组成一个聚合分发单元。这个聚合单元提供共同交付入口和工作流边界，但四个成员继续分别生成制品、维护独立版本与 updater，并可按需安装；共同分发不把它们合并成一个包，也不把它们改成四个独立分发单元。
