...
按已知信息选择入口：已知 ID 或 name 用 `show`；已知 tags、形成时间、关系类型或某条已知直接关系用 `list`；只知道主题、原因或正文措辞用 `search <text>`；需要认识演进时再把完整 ID 交给 `trace`。`list` 与 `search` 可用 `--related-to <selector>` 限定一个本领域目标，并用 `--direction predecessors|successors|both` 选择相对该目标的直接前序、直接后继或两者（默认 `both`）；只给 direction 是参数错误。目标与 relation type 同时出现时必须由同一条边满足。`search` 仅扫描索引选出的正式 Markdown，返回完整 ID、摘要、sourcePath 与命中预览；candidate、资源和索引文件不在范围内。`all`、`any`、`phrase` 分别要求全部词、任一词、同一物理行连续短语：统一 NFKC、忽略大小写并按空白处理查询，前两者可跨物理行，`phrase` 不跨行。它可复用 tags、形成时间和关系条件；`--limit` 只限制返回的命中报告（默认 50、最大 1000）。
