...
仅当用户明确要求**记录、沉淀、创建、更新或审阅**调查时使用。普通调查、排障和问答在当前工作中完成即可，不因可能有用而自动建立报告。
...
创建、原地修正、关系调整、显式剔除或只读审阅都从[skill 入口](../../skills/investigation-report/SKILL.md)选择路径。它定义形成时质量、直接关系判断、写入授权、索引同步与检查顺序；固定契约定义精确命令和机器规则。只读审阅不会为修复索引而写入；索引缺失或过期时应报告这一缺口，而不是把旧投影当作集合事实。
