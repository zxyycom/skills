...
本资源随附于[历史决策对齐状态召回与来源修复](../../recall-historical-decision-alignment.md)，保存该轮 92 个对象的完整采用方向、最终值、历史定位、修复前 SHA-256、区分性依据与逐项局限。关键结论与验证边界由主报告承接；需要复核单条判断时，在本资源中按完整 Decision ID 查找，再读取对应 Git `revision:path`。

## 来源与方法

- 修复前基线：`87c39c9541089cb33b027ba771da9f47c5bc6815`。每项“原始对象”的 SHA-256 对应该基线中的完整 Markdown 原始字节，不是修复后文件的哈希。
- `recorded`：恢复直接历史 frontmatter 已记录的对齐值，不等于本轮独立验证当时实现。
- `semantic`：在列明的同一历史 tree 中核对完整决策与实现、配置、规范或验证材料。
- `inferred`：在用户授权范围内基于本地可得材料合理推断；原始实现不能完整重读，不视为已验证。

### 共同审查与写入规则

- 每条只在独立主审批准后进入本资源的最终值；下列逐项标题中的 alignment 就是该批准结果。为避免机械重复，不在每项重述相同的主审句。
