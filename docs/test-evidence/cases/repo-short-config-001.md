...
Contract:
- 每个仓库维护短命令必须保留类型化映射指定的 package script 实现；`check:test-evidence-catalog` 必须委托项目快照、引用与覆盖 wrapper；`check` 与 `test:check` 必须由 Node 宿主委托 Vibe 入口，不能保留候选 `vibe-check` 入口。
