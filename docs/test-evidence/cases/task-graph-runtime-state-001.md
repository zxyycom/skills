...
Proves:
- info 区分 missing/incompatible，mutation binding 分别返回 `RUNTIME_MISSING` 与 `RUNTIME_INCOMPATIBLE`；`RUNTIME_MISSING` 的 JSON-safe detail 使用无原型对象并保留与 info 相同的安装命令；错误版本没有安装指令，也不被删除。
