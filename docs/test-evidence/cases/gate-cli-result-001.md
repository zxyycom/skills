...
- 调用控制使用 `checks: "effective"`，把规范化公开 flags、内部 activation flags 与唯一目录下的 `machine/`、`progress.log`、`checks/` 一起交给 Vibe；目录名由 UTC timestamp 与 UUID 构成，`gate-incremental.json` 保留 activation 计数、逐 Check 决策、snapshot fallback detail、receipt 发布结果和 release 测试证明状态；reused 与 cold 分别写入机器字段并产生可核对的终端提示。
