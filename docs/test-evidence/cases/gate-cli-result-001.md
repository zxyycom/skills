...
- 每次 CLI invocation 向 Vibe 传入一个 Run AbortSignal；启用 diagnostic flag 时，调用控制只为当前 invocation 启用 `.log/vibe-check` 的 diagnostic logging。completed passed/failed aggregate，以及 cancelled、planning、execution、output 的 invocation failure 分别回显成功创建的 `core-*.log` 与 `scheduler-*.log` 路径。failed aggregate 与 configuration 类 invocation failure 都返回退出码 1、保留 Vibe 状态或类别而不重建 renderer；configuration 因没有 outputs 不回显日志路径。
