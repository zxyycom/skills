...
Contract:
- `check`、`show`、`plan` 与 `finalize` 从目标父目录推导项目约定 Change 根（默认 `changes/`），只接受该根的直接 active member；tombstone children 和嵌套路径不构成公共目标。
...
- 所有拒绝目标的 metadata 保持原样；实际 finalize 不删除目标或在其推导根下创建 tombstone，show 不读取被拒绝目标内容。
