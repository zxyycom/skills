...
Contract:
- 实际 `index stage` 默认使用稳定单行文本，显式全局 `--json` 序列化同一 raw result；Git pending staging 不加载 task-index mutation native runtime。

Proves:
- 成功文本逐字段报告 state、revision、task 数、nextTaskId 与排序后的选择，JSON route 返回同一结构化数据。
