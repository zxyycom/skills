...
Proves:
- 根 help 枚举全部 28 个命令并逐一查询成功，机器可读地公布固定 Node range、mutation runtime 前置以及安装命令；scope create、task create 与 apply 的参数契约可恢复，其中 acceptance 明确为可选重复参数；每次调用只写一个 LF 结尾 JSON。
