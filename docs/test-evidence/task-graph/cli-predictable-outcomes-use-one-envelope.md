...
Proves:
- 合法 binding/reference 可创建和查询，持久字典保留字稳定拒绝，外部非法 ID 返回明确 not-found；atomic reject 的 unknown outcome 保留 possible revision，atomic resolve 不要求读回；各结果保持单 JSON、稳定 error code 与 retryable。
