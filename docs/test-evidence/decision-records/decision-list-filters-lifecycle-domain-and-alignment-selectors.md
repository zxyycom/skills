...
Contract:
- List CLI 必须从持久索引按生命周期、领域与对齐状态返回紧凑定位信息，并验证领域选择器。
Proves:
- active、archived、all、full-time、domain 和 alignment 选择器产生匹配集合；默认只展开 title 与 purpose，不重复 background 和 decision。
