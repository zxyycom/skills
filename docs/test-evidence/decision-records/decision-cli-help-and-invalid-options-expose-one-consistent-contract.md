...
Contract:
- 顶层与子命令帮助必须准确说明默认 check、索引检查/重建、只有完整方向成为当前事实并完成核对后才能对齐，以及归档保留对齐状态；非法参数继续使用参数错误退出契约。
Proves:
- 帮助文本公开 agent-oriented 入口、默认严格检查、sync-index 的只检/写入差异、mark-aligned 的事实基线语义和归档保留 alignment；未知选项、非法深度、非法领域 ID 与缺失必需参数均退出 2。
