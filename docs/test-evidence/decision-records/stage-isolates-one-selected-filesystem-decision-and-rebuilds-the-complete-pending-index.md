...
Contract:
- `stage` 只把显式选择的 filesystem 决策叠加到 revision 基线，以同一目标来源重建完整 pending 索引，并整体替换 pending 决策范围。

Proves:
- 选择 A 后，pending 包含 filesystem A 与 revision B，先前 pending 中的 filesystem B 被移出决策范围。
- pending 索引包含完整 A/B 集合，且 `sourceRevision` 与该 pending 领域目录和 Markdown 来源一致。
