...
Contract:
- 报告资源链接只能原样书写为 `../_resources/<resource-id>`，不能借助 Markdown 转义或字符实体形成解码别名；资源 ID 必须是小写、规范、不越界且不含查询、片段或百分号编码。

Proves:
- 上级路径与大写 ID 分别命中包含原链接目标的非规范 ID 诊断，绝对路径、query、fragment、百分号编码和反斜杠分别命中包含原链接目标的固定前缀诊断。
- 字符实体解码目标与 Markdown 转义目标均命中原始目标必须按字面书写的专用诊断。
