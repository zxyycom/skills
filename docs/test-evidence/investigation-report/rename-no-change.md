...
Entry:
- `tools/investigation-report/tests/rename.test.ts > Investigation rename is a zero-write no-op when ID name path and owner already match`
- `bun test --test-name-pattern="^Investigation rename is a zero-write no-op when ID name path and owner already match$" ./tools/investigation-report/tests/run.ts`
