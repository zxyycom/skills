...
Contract:
- Git 测试 fixture 只提交普通原始文件；启动 helper 必须复制原始树，在 case 私有目录初始化真实 `main` Git 仓库、固定本地配置并提交基线。

Proves:
- 原始 fixture 不含 `.git` 或 workspace 绝对路径，物化后的仓库使用 `main` 与 `core.autocrlf=false`，且具有相同基线 revision。
