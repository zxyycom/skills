...
Proves:
- 原始 fixture 不含 `.git` 或 workspace 绝对路径，物化后的仓库使用 `main`、`core.autocrlf=false` 与调用方给定的本地 Git identity，并具有相同基线 revision。
- 屏蔽 system/global Git 配置及进程 identity 后，物化仓库仍可依靠自身本地配置完成后续 commit。
