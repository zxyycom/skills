...
try {
  ...
  await Promise.all([
    ...
    fs.writeFile(unmanagedDeclarationPath, "export type Manual = true;\n", "utf8"),
  ...
  assert.equal((await fs.lstat(linkedDeclarationPath)).isSymbolicLink(), true);
} finally {
  await fs.rm(generatedDeclarationsTempRoot, { force: true, recursive: true });
}
});
