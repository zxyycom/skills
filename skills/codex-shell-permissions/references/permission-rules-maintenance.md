...
3. 目标程序已经启动，但文件或目录操作返回权限错误：先按 [路径权限诊断](path-permission-diagnosis.md) 定位执行身份、permission profile、文件系统权限、程序请求权限或隐式配置路径；shell rule 不会改写这些权限。
4. 同类只读命令反复被误判为 prompt/block，且参数边界稳定：可以提出补 allow 建议，等待用户确认后再改。
5. 命令能包住任意子命令、写入、删除、安装、下载、登录、启动服务或改配置：默认保留 prompt。
6. 用户要求长期调整权限行为，或明确批准维护 rules，才进入 rule 修改流程。
...
7. 修改后用同一命令 token 重新跑 `execpolicy check`，再根据命令风险和当前授权决定是否真实执行；不能安全实跑时，明确报告 runner 行为尚未验证。
...
5. 真实 runner 的验证结果，或者因风险或授权边界未执行的原因。
6. 是否需要重启、热加载或重新打开会话。
