...
const MANAGED_INDEX_HEADINGS = new Set([
  "## 活动决策",
  "## 决策清单"
]);
const IMPACT_AREA_PATTERN = /^[a-z0-9]+(?:-[a-z0-9]+)*$/;
const DECISION_FILE_PATTERN = /^(\d{6})-([a-z]+)-([a-z0-9]+(?:-[a-z0-9]+)*)\.md$/;
const DECISION_RELATIVE_PATH_PATTERN = /^(?:archive\/)?[a-z0-9]+(?:-[a-z0-9]+)*\/\d{6}-[a-z]+-[a-z0-9]+(?:-[a-z0-9]+)*\.md$/;
...
function parseArgs(argv) {
  ...
  if (!new Set(["check", "list", "sync-index"]).has(command)) {
...
function parseDecisionFileName(fileName) {
  const match = fileName.match(DECISION_FILE_PATTERN);
  if (!match) {
    return null;
  }
...
async function validateDecisionLink(options) {
  ...
  if (!DECISION_RELATIVE_PATH_PATTERN.test(relativeTarget)) {
...
async function validateDecisionBody(options) {
  ...
  if (fileStatus && !DECISION_STATUSES.has(fileStatus)) {
  ...
  const expectedSectionSet = new Set(SECTION_ORDER);
  ...
  for (const sectionHeading of REQUIRED_SECTIONS) {
  ...
  let previousOrder = -1;
  for (const section of sections) {
    const currentOrder = SECTION_ORDER.indexOf(section.heading);
    if (currentOrder < 0) {
      continue;
    }
  ...
  if (statusSection) {
    ...
    if (bodyStatus && !DECISION_STATUSES.has(bodyStatus)) {
...
async function scanArchive(options) {
  ...
  for (const entry of archiveEntries) {
    ...
    areaIds.add(areaId);
    if (!IMPACT_AREA_PATTERN.test(areaId)) {
...
export async function scanDecisionRecords(options = {}) {
  ...
  for (const entry of rootEntries) {
    ...
    if (entry.isFile()) {
      if (!REQUIRED_ROOT_FILES.has(entry.name)) {
...
function expectedIndex(scan) {
  ...
  const headings = parseSections(index).filter(
    (section) => MANAGED_INDEX_HEADINGS.has(section.heading)
  );
  const errors = [];
  let prefix;
...
export async function validateDecisionRecords(options = {}) {
...
function selectedStatuses(args) {
  if (args.all) {
    return new Set(DECISION_STATUSES);
  }
...
async function runList(args) {
  ...
  const records = scan.records
    .filter((record) => statuses.has(record.fileStatus))
    .sort((left, right) => {
