...
| 状态 | 含义与使用方式 |
| --- | --- |
| `candidate` | 尚未建立的候选，先完成正文、语义审核和建立授权判断。 |
| `active + aligned` | 完整方向已成为当前事实并经过核对，作为当前基线。 |
| `active + unaligned` | 已确认的未来方向，约束相关方案选择；只有当前任务明确纳入交付时才实施。 |
| `archived` | 退出当前依据，保留最后对齐状态与演进历史。 |

对齐状态作用于完整方向，不产生实施授权、任务或优先级。CLI readiness 只证明结构与正文的机械准备情况，语义审查和授权另行判断。
...
已建立记录的生命周期和关系通过 CLI 事务维护，索引从 Markdown 派生。需要只读预演时，`activate/evolve --preflight` 使用本次完整参数验证；正式执行仍须重新提供参数并重新验证。
...
1. 手工修改已建立 Markdown、怀疑索引陈旧或准备维护时，先运行严格 `check`。确认合法来源变化与范围后运行 `sync-index`；使用 `--select` 时仍验证完整集合，并仅接纳显式选中 ID 的变化，添加 `--write` 才发布该投影。
