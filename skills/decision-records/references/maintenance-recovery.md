...
本手册只在 CLI 无法使用、索引缺失或损坏、写入中断，或严格 `check` 失败且普通诊断不足时读取。决策语义和维护不变量以 [决策记录规则](decision-record-rules.md) 为准。
...
4. 权威来源有效而索引缺失、无法解析或陈旧时，按“重建索引”处理。
5. 状态或关系命令疑似中断、或报告恢复失败时，比较权威 Markdown、索引和可用的最后可信版本，确认是否形成完整一致的旧状态或新状态。
