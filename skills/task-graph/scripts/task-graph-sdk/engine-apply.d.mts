/*
 * Generated task graph SDK TypeScript declaration. Do not edit this file directly.
 * Repository: https://github.com/zxyycom/skills
 * Maintained source: https://github.com/zxyycom/skills/blob/main/tools/task-graph/src/engine-apply.ts
 * Source path: tools/task-graph/src/engine-apply.ts
 * Skill source directory: https://github.com/zxyycom/skills/tree/main/skills/task-graph
 * Rebuild: bun run sync:task-graph-cli
 */
import type { IndexMutation } from "./engine-content.mjs";
import type { TaskGraphApplyRequest, TaskGraphApplyResult, TaskIndex } from "./types.mjs";
export declare function applyTaskGraphOperations(current: TaskIndex, requestInput: TaskGraphApplyRequest, now: Date): IndexMutation<TaskGraphApplyResult>;
