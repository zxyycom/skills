...
2. 先调用 index 信息和完整检查命令，读取当前 revision。索引不存在时使用 `init` 创建规范空索引；索引已经存在、损坏或 schemaVersion 未知时不得手写、覆盖或隐式迁移。
3. 按用户或宿主提供的显式 scope ID、唯一 scope key 或 binding 恢复开放 scope。只有没有匹配 scope 时才创建；多项匹配、唯一性冲突或信息不足时保持只读并暴露缺口。
...
1. 每轮调度先读取 actionable 投影及其 revision，并按需读取 task trace。`actionable` 只表示当前存在合法下一动作：叶子任务可以 `claim`，已满足门禁的父任务可以 `complete`。
...
3. 租约过期后不要继续普通 lease 操作。读取当前 task 与 trace，使用匹配 lease ID 进入受控 `recover`；活动租约只有在当前授权足以覆盖强制接管时，才携带 revision、`force` 和原因提前恢复。
...
2. 上下文恢复后重新执行 index 检查、scope 定位和 actionable 查询。对 `running` 或 `recovery-needed` task 先确认实际执行者和 lease 状态，不能因当前对话不记得它就释放或覆盖。
...
2. 只有调用方明确确认结果已经交付后才执行 scope close 或显式 scope GC。第一版不删除单个 task，也不运行后台或按时间静默清理。
