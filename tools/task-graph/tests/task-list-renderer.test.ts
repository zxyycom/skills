...
function constraint(sourceTaskId: string, targetTaskId: string): TaskConstraintSource {
  return {
    targetTaskId,
    sourceTaskId,
...
function dictionary(items: readonly TaskListItem[]): Record<string, TaskListItem> {
...
test("task-list renderer serializes failures with sorted JSON details", () => {
  assert.equal(renderTaskListResult({
    ok: false,
...
test("task-list layout uses actual IDs with stable track layer and parent-path order", () => {
  ...
  assert.equal(renderTaskListResult(success([
    isolated,
    grandchild,
    dependency,
    child,
...
test("task-list renderer fails closed when invalid projection relations reach layout", () => {
  const cases = [
    ...
    },
    {
      ...
      result: success([item("task-000001", {
...
test("task-list renderer displays every causal blocker kind in deterministic order", () => {
  ...
  const work = item(taskId, {
    blockers: [
      blocker({ ...base, kind: "descendant-lease", relatedTaskId: "task-000005", state: "recovery-needed" }),
      blocker({ ...base, kind: "dependency-failed", relatedTaskId: "task-000004", state: "failed" }),
      blocker({ ...base, kind: "dependency-cancelled", relatedTaskId: "task-000003", state: "cancelled" }),
      blocker({ ...base, kind: "ancestor-terminal", relatedTaskId: "task-000002", state: "succeeded" }),
      blocker({ ...base, kind: "all-children-cancelled", relatedTaskId: "task-000001", state: "cancelled" })
    ],
    title: "Work"
  });
  const output = renderTaskListResult(success([
    item("task-000001"),
    item("task-000002"),
    item("task-000003"),
    item("task-000004"),
...
test("task-list renderer omits blockers recoverable from full task data", () => {
  ...
  const base = { taskId, sourceTaskId: taskId, inheritancePath: [taskId] };
  const output = renderTaskListResult(success([item(taskId, {
    blockers: [
      blocker({ ...base, kind: "control-candidate", relatedTaskId: taskId, state: "candidate" }),
      blocker({ ...base, kind: "control-waiting", relatedTaskId: taskId, state: "waiting" }),
      blocker({ ...base, kind: "control-paused", relatedTaskId: taskId, state: "paused" }),
      blocker({ ...base, kind: "dependency-incomplete", relatedTaskId: taskId, state: "waiting" }),
      blocker({ ...base, kind: "child-incomplete", relatedTaskId: taskId, state: "waiting" })
    ],
    title: "Recoverable"
  })]), { columns: 80 });
...
test("task-list renderer maps exclusion-running blockers to active mutex tokens", () => {
  ...
  const output = renderTaskListResult(success([
    item(taskId, {
      blockers: [
        blocker({ ...base, kind: "exclusion-running", relatedTaskId: "task-000003", state: "running" }),
        blocker({ ...base, kind: "exclusion-running", relatedTaskId: "task-000002", state: "recovery-needed" }),
        blocker({ ...base, kind: "exclusion-running", relatedTaskId: "task-000002", state: "running" })
      ],
      title: "Blocked"
    }),
    item("task-000002"),
...
test("task-list renderer preserves fixed node field order", () => {
  ...
  const work = item(taskId, {
    ...
    reason: "because",
    blockers: [
      blocker({ ...base, kind: "dependency-failed", relatedTaskId: "task-000002", state: "failed" }),
      blocker({ ...base, kind: "exclusion-running", relatedTaskId: "task-000003", state: "running" })
    ],
    title: "Work 中文🚀"
  });
...
test("task-list renderer keeps actual mutex endpoints across same and different tracks", () => {
  ...
  const sameTrack = renderTaskListResult(success([
    item("task-000002", {
      exclusionTargets: ["task-000001"],
...
test("task-list node form switches at 79 and 80 columns without measuring title width", () => {
  ...
  assert.equal(renderTaskListResult(success([single]), { columns: 80 }), [
  ...
  assert.equal(renderTaskListResult(success([single]), { columns: 79 }), [
    "TASK LIST tasks=1 tracks=1 actionable=1 running=0 recovery-needed=0 mutex-blocked=0",
    "",
    "TRACK T01 tasks=1",
...
test("task-list node form switches when any relation list exceeds three endpoints", () => {
  ...
  const cases = [
    {
      label: "needs",
      output: renderTaskListResult(success([
        item(taskId, {
    ...
    },
    {
      ...
      output: renderTaskListResult(success([
        item(taskId, {
          blockers: [
            blocker({ ...base, kind: "ancestor-terminal", relatedTaskId: "task-000002", state: "cancelled" }),
            blocker({ ...base, kind: "dependency-cancelled", relatedTaskId: "task-000003", state: "cancelled" }),
            blocker({ ...base, kind: "dependency-failed", relatedTaskId: "task-000004", state: "failed" }),
            blocker({ ...base, kind: "descendant-lease", relatedTaskId: "task-000005", state: "running" })
          ],
    ...
    },
    {
      ...
      output: renderTaskListResult(success([
        item(taskId, {
          blockers: targets.map((relatedTaskId, index) => blocker({
            ...base,
        ...
        ...targets.map((target, index) => item(target, {
  ...
  assert.match(cases[2]?.output ?? "", /^TASK LIST .* running=2 recovery-needed=2 mutex-blocked=1\n/u);
});
...
test("task-list renderer normalizes inherited symmetric exclusions into unique pairs", () => {
  ...
  const output = renderTaskListResult(success([right, child, ancestor]), { columns: 80 });
...
test("task-list track labels keep at least two digits without a two-digit limit", () => {
  const items = Array.from({ length: 100 }, (_, index) => item(
    `task-${String(index + 1).padStart(6, "0")}`,
