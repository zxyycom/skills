...
import { TaskGraphService, cancelTask, removeTasks } from "../src/index.ts";
...
test("task removal reports terminal and graph-boundary blockers without mutation", () => {
  ...
  for (const [taskId, blockerKind] of selections) {
    const error = expectTaskGraphError(
      () =>
        removeTasks(index, {
          expectedRevision: index.revision,
          taskIds: [taskId],
    ...
    const blockers = error.details.blockers as unknown as Array<{
      kind: string;
    }>;
...
test("CLI exposes batch task removal and no scope command", async () => {
  await withTempWorkspace(async (root) => {
    ...
    const helpExit = await runTaskGraphCli(["--root", root, "--help"], {
    ...
    if (help.ok) {
      ...
      assert.equal(
        help.data.commands.some((command) => command.startsWith("scope ")),
        false
      );
    }

    const scopeChunks: string[] = [];
    const scopeExit = await runTaskGraphCli(["--root", root, "scope", "list"], {
    ...
    if (!scopeResult.ok)
      assert.equal(scopeResult.error.code, "ARGUMENT_INVALID");
  });
});
...
test("task index mutations leave Git staging, commits, and ignore policy caller-owned", async () => {
  await withTempWorkspace(async (root) => {
    ...
    await execFileAsync(
      "git",
      ["config", "user.email", "task-graph@example.invalid"],
      {
        cwd: root,
        windowsHide: true
      }
    ...
    await execFileAsync(
      "git",
      ["add", ".gitignore", "docs/task-graph/task-graph-index.json"],
      {
        cwd: root,
        windowsHide: true
      }
    ...
    await service.apply({
      expectedRevision: 0,
      operations: [
        { kind: "create-task", content: taskContent("git boundary") }
      ]
    });

    await fs.writeFile(
      `${service.store.indexPath}.tmp-test`,
      "temporary\n",
      "utf8"
    );
    const status = (
      await execFileAsync("git", ["status", "--porcelain=v1", "--ignored"], {
        cwd: root,
        encoding: "utf8",
        windowsHide: true
      })
    ...
    assert.doesNotMatch(
      status,
      /^M  docs\/task-graph\/task-graph-index\.json$/mu
    );
    assert.match(
      status,
      /^\?\? docs\/task-graph\/task-graph-index\.json\.tmp-test$/mu
    );
