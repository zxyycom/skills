...
import {
  defaultVerificationEvidenceConfigPath,
  verificationEvidenceConfigSchema,
  verificationEvidenceConfigSchemaVersion
} from "./schemas.ts";
import type {
...
export async function loadVerificationEvidenceConfig(
  workspaceRoot: string,
  requestedConfigPath?: string,
  providedConfig?: unknown
): Promise<LoadedVerificationEvidenceConfig> {
  ...
  if (value === undefined) {
    try {
      ...
      if (isMissingFileError(error) && requestedConfigPath === undefined) {
        value = { schemaVersion: verificationEvidenceConfigSchemaVersion };
      } else {
  ...
  const parsed = v.safeParse(verificationEvidenceConfigSchema, value);
  if (!parsed.success) {
