...
export type ParsedVerificationCase = {
...
export function collectVerificationCases(
  text: string,
  ...
  const entries: ParsedVerificationCase[] = [];
...
function parseVerification(value: string): VerificationKind | null {
  return verificationKinds.find((verification) => verification === value) ?? null;
}
