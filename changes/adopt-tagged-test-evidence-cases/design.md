...
- 本 Change 与 Git 历史中的 `adopt-tagged-decision-records` 源自同一组早期 draft；Decision Records 已明确把文件名唯一性限定在自身集合。原 Investigation Report draft 已删除，Git 历史中的 `establish-investigation-report-evolution` 同样不建立跨资源全局 ID namespace。本 Change 不能再把三个集合的合并文件数或全局 basename 唯一性当作共享前置，Test Evidence 的文件身份与冲突门禁需要在自身 Plan 内重新收敛。
