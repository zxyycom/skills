...
- 以 `add-relation-summaries` 的完成 revision 为硬前置，只消费该 Change 已交付并测试的可选 `relations[].summary` 索引投影；metadata 将非空 summary 作为该来源记录的独立 segment，并用 `matchedRelations` 返回命中的 `{ type, target, summary }`。
...
- `add-relation-summaries` 未完成、其目录仍为 active Change，或无法核对其完成 revision、最终 relation projection、测试和生成制品时，本 Change 停在 Readiness，不实现或修改 summary 的来源、索引、Schema 或迁移。
...
- 已核对 `add-relation-summaries` 的完成 revision、最终 relation projection 与验证证据；本 Change 只消费其已发布 summary。缺失/空 summary 不命中，命中 summary 只返回来源记录及精确 `matchedRelations(type,target,summary)`，不因 target/type 文本或 target 记录而命中。
...
- `changes/add-relation-summaries/` 及其完成 revision 是 relation `summary` 的来源/写回、graph/lifecycle、索引投影、Schema、definition version、rename、迁移和验证 owner；本 Change 只在其完成后读取已发布投影。
