...
- [ ] 1.2 扩展共享版本管理公共类型与首版 Git 实现，提供字面路径范围的完整 `pending` 文件集合替换、范围外保留、输入校验、稳定错误映射、读回核对和可处理失败恢复；同步 `tools/shared/version-control.md` 的当前事实。
...
- [ ] 2.5 用代表性 agent 任务审阅更新后的行为文档，确认 AI 能区分 `filesystem` 生命周期事务与 `pending` 事务，推导选择集和范围排除，定位共享 owner 与失败出口，并准确说明“本决策不要求 Git 以外实现，且公共边界不透传 Git”。
