...
- [x] 0.5 确认长期决策的 JSON 解析边界与本 change 一致，两条后继长期决策保持 `active + unaligned`，且三个 artifact 没有阻塞开放问题。
...
- [ ] 1.3 改造 index JSON 解析边界，安全处理原型敏感 id，并为 schema v2、id 集合不一致和来源 revision 非法提供稳定诊断。
...
- [ ] 2.2 验证 schema v3 序列化确定且不依赖输入顺序、schema v2 拒绝，以及 `__proto__`、`constructor` 等特殊 id 的 parse/build/get/query 往返。
