...
本设计把“精确同步”定义为对允许接纳的来源变化做 ID scope 门禁：运行时仍证明完整当前投影，成功索引仍全局新鲜，但任何未选择变化都会阻断写入。
...
- 本 Change 依赖 [`存储无关纯 ID`](../separate-domain-ids-from-storage-details/)完成；CLI 和 shared runtime 的 `selectedIds` 不接受文件名或目录路径。
...
- 让 add/update/delete/rename 使用一套对称的 old/current ID union 语义。
- 在共享 runtime 固定公共 scope 和诊断，在各领域 owner 固定 CLI 与领域 ID 校验。

非目标：

- 不以减少文件读取、解析或校验数量为目标，不增加 `readSelected(ids)` 来源接口。
- 不合并工作区同步和 Git pending staging，不让 `sync-index --id` 自动暂存任何文件。
...
公开 TypeScript 表面采用等价的 discriminated union；领域 CLI 以是否出现一个或多个 `--id` 映射 scope。不带 `--id` 是 `all`，至少一个 `--id` 是 selected；空字符串、控制字符、重复项或不符合领域纯 ID grammar 的输入作为参数错误，在读取或写入索引前失败。规范结果按 locale 无关词法顺序报告 `selectedIds` 与 `changedIds`。

#### Selected baseline 与完整 candidate

selected sync 按以下顺序执行：

1. 校验 runtime definition、scope、context 和 index path。
...
6. 对 baseline/candidate 的 `entries` 与 `sourceRevision.entries` key union 排序，若某 ID 的 entry 存在性/规范状态或 per-ID revision 任一不同，则把它加入 `changedIds`。
7. 每个 selected ID 必须至少存在于 baseline 或 candidate 一侧；因此删除可选择旧 ID，新增可选择新 ID。不存在于两侧的 ID 是 selection error。
8. 若 `changedIds` 不是 `selectedIds` 的子集，返回全部 `unselectedChangedIds` 并零写入；selected 中未变化的额外 ID不失败。
9. 门禁通过后，check mode 只报告 current 或 scoped stale 状态而不写入；write mode 使用现有原子发布与回读验证写入完整 candidate。
...
| --- | --- |
| 新增 | 新 ID |
| 修改内容、状态或 sourcePath，但 ID 不变 | 当前 ID |
| 删除 | 旧 ID |
...
`StateIndexSyncResult` 增加明确的 scope 信息，并在适用时返回排序后的 `selectedIds`、`changedIds`。稳定诊断至少区分：selection-invalid、selected-id-missing、unselected-changes、collection-changed 和 selected-baseline-invalid；每个 ID 级诊断携带 `stateId`，未选择变化完整列出而不是只报告第一项。
...
```text
sync-index [--id <pure-domain-id>...] [--write]
```

Decision 与 Investigation 接受各自纯 record ID，Test Evidence 接受 Case ID。领域层先完成 ID grammar/CLI 参数错误分类，再把 canonical IDs 交给 shared runtime；runtime 不了解 `.md`、Change root、Case 前缀等领域存储规则。

领域命令继续遵循自己的来源根、warning、lock 和输出 envelope。文档必须明确：`sync-index --id` 从来源更新 workspace index；`stage-index <id...>` 从 workspace index 组合 Git pending，二者不能互相替代。
...
- **Decision Records：** `sync-index` 增加纯 Decision ID 选择，并继续验证完整 Decision 集合及关系；生命周期事务已有自己的完整索引提交语义，不被改成调用局部同步。
- **Investigation Report：** selected sync 继续在 collection mutation lock 内验证完整正式报告、关系和资源；candidate 仍不是正式索引 entry，不能作为 `--id` 绕过 publish。
...
| 纯 ID 迁移尚未完成 | Readiness 硬性依赖前置 Plan 完成；不先实现带 `.md` 的临时 selected API |

## Open Questions

无。selected scope 的正确性、baseline 要求、check/write 行为、CLI 外形和领域范围已经确定；实现若需要改变 partial-freshness 或集合 metadata 规则，必须先修订本设计。
