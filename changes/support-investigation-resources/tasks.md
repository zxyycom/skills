...
先建立“报告引用资源”的最小闭环，再补齐资源完整性、索引投影、分发与验证；索引和哈希只服务于引用能够可靠恢复。

## Readiness

- [x] 0.1 确认本 change 的核心结果是一份 H3 调查报告可以通过固定本地链接引用至少一项随附资源；多资源、共享资源、索引关系和 SHA-256 都围绕该结果展开。
...
- [ ] 1.2 扩展 Markdown AST 解析、报告投影、领域类型和主题 state 构建，从每份报告恢复资源链接，拒绝非法结构、空展示文字和报告内重复资源。
...
- [ ] 2.2 在临时调查集合中让一份报告引用一项资源，再覆盖多报告、共享文本资源和二进制资源；运行 `sync-index`、默认 `check`、局部 `check` 与 `list`，核对报告关系和 SHA-256，并验证资源变化会使旧索引失效且诊断定位资源 ID。
