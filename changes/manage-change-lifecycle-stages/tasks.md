...
- [x] 1.1 在 `tools/change-plan/` 实现 `.change-plan.json` 判别联合、读取写入、公开类型和 active metadata 检查。
- [x] 1.2 按阶段改造 artifact 与任务检查：draft 接受最小 proposal，plan、implementation 和 shelved 使用完整三文件，并提供任务区段进度。
- [x] 1.3 在共享版本控制层实现领域无关的 first-parent revision、numstat、路径和错误契约，并在 Change Plan 中实现当前 change 目录排除、距离聚合与阈值判断。
...
- [x] 1.6 扩展 list、show、check、stage filter、文本与 JSON 输出、公共 API、CLI 帮助和退出码。
- [x] 1.7 从运行时公开导出和 metadata schema 机械生成自包含 MJS、可达 SDK 声明树、JSON Schema、公开 metadata 类型和 source map，不保留独立 api 声明源。
...
- [x] 1.10 新增或调整共享版本控制契约及 Change Plan 的元数据、阶段、Git 距离、命令、查询、archive、迁移和分发 API 原生测试，并同步各自测试证据 case 与派生索引。

## Verification

- [x] 2.1 验证四阶段 metadata、draft 最小内容、其他阶段完整三文件、任务区段进度和 active/archived 边界。
- [x] 2.2 在临时 Git 仓库验证 `3/1000` 保持 current，`4/1001`、`9/小 churn`、`1/3000` 成为候选，并验证零项目提交、空提交、二进制提交、只改 change 目录和计划内容变化。
- [x] 2.3 验证 plan、implement、显式 shelve、reconcile、resume 与 archive 的合法路径，以及 candidate、待复核 plan 和 shelved 的推进门禁。
- [x] 2.4 验证 list、show、check、stage filter、紧凑候选摘要、JSON evidence、公共 API 和 `0/1/2` 退出码。
