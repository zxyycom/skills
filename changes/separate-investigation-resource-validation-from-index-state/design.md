...
- **被引用资源**：默认全量检查中至少被一份合法主题报告声明的资源 ID。完全未出现在全量合法引用集合中的可见成员是完全未引用资源。
...
实现修改 `tools/` 源码、公开声明源和测试后，从维护源码重新生成分发 CLI、source map、TypeScript 声明与 JSON Schema，并把 `investigation-report` skill version 从 `15` 提升到 `16`。测试修改按 Test Evidence Review 维护一入口一 case，移除或改写仍声称未引用资源、资源 revision 或资源 metadata 会阻塞的过时 case，再同步派生索引。具体执行与验收状态由 `tasks.md` 承接。
...
| scoped check 不能证明全局 owner 与未引用资源状态 | CLI 和契约明确其证明范围；同步前仍使用默认全量校验 |
