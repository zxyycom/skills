...
- [ ] 2.2 覆盖未知 topic、重复或非法 id、两份索引都不存在的 id、无仓库、同索引既有 pending、revision 冲突和写入恢复，并回归 topics/check/sync/list/show。
- [ ] 2.3 确认命令不修改 filesystem、不读取或暂存 topic 表、case Markdown 或代码，其他待提交路径保持不变。
