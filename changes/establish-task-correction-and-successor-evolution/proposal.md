...
本 proposal 规划 task-graph 的受控任务纠正与后继演进边界，并把错误终态纠正收敛为一个可追溯、不会绕过 lease 与版本锚点的显式流程。

## Why

### Status and Reading Contract

本 Change 当前处于 `plan` 阶段。本文、[design.md](design.md) 和 [tasks.md](tasks.md) 中的 `correct`、`terminalCorrections` 与相关输出都是**待实施的目标契约**，不是当前 runtime 已具备的行为。

三个 artifact 的职责如下：

| Artifact | 权威内容 | 使用方式 |
| --- | --- | --- |
| `proposal.md` | 目标结果、范围、成功标准和受影响 owner | 判断本 Change 应该交付什么，以及某项工作是否越界 |
| `design.md` | 生命周期选择、公开契约、事务门禁、兼容边界和取舍 | 实现或审阅前恢复唯一目标设计；其中的状态路由是操作入口 |
| `tasks.md` | 实施顺序、验证节点和完成证据 | 在已确认设计下推进，不从任务措辞反推新的产品规则 |

当前已经生效的 task-graph 行为仍以 `skills/task-graph/SKILL.md`、`docs/skills/task-graph.md` 和 `tools/task-graph/src/` 为准。实施完成后，稳定行为与长期理由进入这些 owner 和对应决策记录；本 Change 只保留实施上下文。

### Problem

当前 task-graph 已经为未终态任务提供内容、control 与关系 mutation，为运行任务提供 lease 所有权下的 release、fail、cancel 与 complete，并让 failed task 通过 retry 回到 idle。`succeeded` 与 `cancelled` 则会封闭普通 mutation，以防内容、拓扑和 result 被静默改写。
...
如果用一个通用 amend、任意 state patch 或 metadata-only result 更新覆盖三类情况，就会让活动 lease、父子完成门禁、依赖成功证据、排斥关系和 Git 版本锚点失去可信度。相反，如果所有变化都只能创建新任务，错误的 succeeded 仍会继续满足下游依赖并冒充当前事实。因此需要保留普通 mutation 的现有职责，只为真正的终态事实错误增加一个窄而显式的纠正入口。
...
9. 目标 task-graph 测试、生成漂移检查、决策记录严格检查、test-evidence 严格检查和仓库完整检查通过；实施任务 `task-000044` 与重叠的 task ID、tags/find plans 串行协调，后续计划在本 Change 集成后重新确认版本与共享 owner，不并行写入同一实现表面。
...
- `docs/decisions/task-graph/` 与 `docs/decisions/decision-index.json`：保存“错误终态显式纠正、当前 result 与 correction evidence 分离、正确完成后使用新 task”的长期判断；生命周期和关系通过 decision-records owner 维护。
- `changes/support-explicit-task-ids/` 与 `changes/add-task-tags-and-find/`：只作为共享 owner 的实施协调输入；承接本 Change 实施的 `task-000044` 与二者串行，本 Change 集成后由后续计划重新确认 CLI minor version、skill version 与 Schema/type 合并方式。
