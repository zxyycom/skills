...
为实现显式 task ID，核心调整是建立自动、显式与持久 ID 的统一身份契约，以正向便携字符集约束请求，并让创建、占用回退、结果与 alias 始终表达最终实际 ID：
...
持久身份域扩大后，索引兼容性、CLI 协议、validator 消费面、生成 owner 与既有“不复用”长期判断都必须随之对齐。以下决定处理这些连锁影响，不增加 rename、迁移或第二套身份系统：
